      ...
      <div class="todo-app-area">
        <div class="todo-app-list-wrapper">
          <div class="todo-app-list">
            ...
            <!-- Notes list -->
            <perfect-scrollbar
              class="todo-task-list list-group"
              ref="scrollbar"
            >
              <ul
                class="todo-task-list-wrapper media-list"
                v-if="list.length > 0"
              >
                <!-- Note -->
                <li
                  ...
                  :key="i"
                  class="todo-item"
                  v-bind:class="{ completed: todo.filters.completed }"
                  data-toggle="modal"
                  data-target="#taskModal"
                  @click="setTodo(todo.id)"
                >
                  <div
                  ...
                  >
                    <div class="todo-title-area d-flex align-items-center">
                      ...
                      <div class="title-wrapper d-flex">
                        <div class="vs-checkbox-con">
                          <input
                            @click.stop="toggleFilter(todo.id, 'completed')"
                            type="checkbox"
                            v-model="todo.filters.completed"
                          />
                      ...
                      <div class="chip-wrapper">
                        <div
                          class="chip mb-0"
                          v-for="(label, j) in todo.labels"
                          :key="j"
                        >
                          <div class="chip-body">
                            <span class="chip-text" data-value="Frontend"
                              ><span
                                class="bullet bullet-primary bullet-xs"
                              ></span>
                              {{ printLabel(label) }}</span
                            >
                          </div>
                        </div>
                      </div>
                    ...
                    <div class="float-right todo-item-action d-flex">
                      <a
                        class="todo-item-favorite"
                        @click.stop="toggleFilter(todo.id, 'starred')"
                        ><i
                          class="feather icon-star"
                          :class="todo.filters.starred ? 'warning' : ''"
                        ></i
                      ></a>
                      <a
                        class="todo-item-info"
                        @click.stop="toggleFilter(todo.id, 'important')"
                        ><i
                          class="feather icon-info"
                          :class="todo.filters.important ? 'success' : ''"
                        ></i
                      ></a>
                      <a
                        ...
                        @click.stop="
                          confirmDelete(
                            todo.id,
                            'trashed',
                            todo.filters.trashed
                          )
                        "
                      >
                        <i
                          class="feather icon-trash"
                          :class="{ danger: todo.filters.trashed }"
                        ></i>
                      </a>
                    </div>
                  </div>
  ...
  methods: {
    ...
    async toggleFilter(id, filter, trashed) {
      ...
      if (!trashed) {
        ...
        const response = await axios.post(`${URL}/todos/delete-todos`, {
          todo_id: id,
        });
